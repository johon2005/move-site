...
class Arrow {
  constructor(x, y, target) {
    ...
    let tailLength = Math.floor(
      Util.map(this.size, this.min_size, this.max_size, 4, 16)
    );
    this.tail = [];
  ...
  update() {
    ...
    f.setMag(
      Util.map(Util.clamp(d_f_target, 0, 400), 0, 400, 0, this.topSpeed * 0.1)
    );
    this.addForce(f);
